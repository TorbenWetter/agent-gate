...
agentpass v1 has hardcoded tool definitions (Home Assistant only) spread across `executor.py` (TOOL_SERVICE_MAP), `engine.py` (SIGNATURE_BUILDERS, HA_IDENTIFIER_RE), `services/homeassistant.py` (HTTP endpoint mapping), and `config.py` (HomeAssistantConfig). This spec replaces all of that with:
...
3. **CLI client** — `agentpass request <tool> key=value` for any agent with shell access (replaces mcporter for OpenClaw)
4. **Tool discovery** — `agentpass tools` queries the gateway for available tools and their schemas
...
- [ ] FR7: CLI subcommand `agentpass request <tool> [key=value ...]` sends one-shot tool requests
  ...
  - AC: Gateway URL from `--url` flag or `AGENTPASS_URL` env var
  - AC: Token from `--token` flag or `AGENT_TOKEN` env var
  - AC: Timeout via `--timeout` flag (default: 900s)

- [ ] FR8: CLI subcommand `agentpass serve` starts the gateway server (default when no subcommand)
  - AC: `agentpass --insecure` still works (backward compatible)
  - AC: `agentpass serve --insecure` also works
  - AC: Existing deployments unaffected

- [ ] FR9: CLI subcommand `agentpass tools` lists available tools from the gateway
...
- [ ] FR10: CLI subcommand `agentpass pending` retrieves offline results
...
3. **Tool discovery:**
   - `agentpass tools --url wss://gateway:8443 --token mytoken`
...
4. **CLI pending results retrieval:**
   - `agentpass pending --url wss://gateway:8443 --token mytoken`
...
| No `--url` and no `AGENTPASS_URL` for CLI request | Error to stderr, exit 3                                         |
...
- `src/agentpass/config.py` — New dataclasses, generic service loading, tool YAML loading
- `src/agentpass/registry.py` — NEW: ToolRegistry mapping tool names → definitions + services
- `src/agentpass/engine.py` — Registry-aware signature building + arg validation
- `src/agentpass/executor.py` — Registry-based dispatch (remove TOOL_SERVICE_MAP)
- `src/agentpass/services/http.py` — NEW: GenericHTTPService (replaces homeassistant.py)
- `src/agentpass/services/homeassistant.py` — Deprecated compatibility wrapper
- `src/agentpass/cli.py` — NEW: CLI request/tools/pending commands
- `src/agentpass/__main__.py` — Subcommand routing, generic service init
- `src/agentpass/server.py` — New `list_tools` method handler
...
agentpass
  ...
  |     --url URL          (or AGENTPASS_URL env var)
...
- `src/agentpass/config.py` — Add ArgDefinition, RequestDefinition, ResponseDefinition, ToolDefinition, AuthConfig, HealthCheckConfig, ErrorMapping, ServiceConfig. Add `load_tools_file()`.
- `src/agentpass/registry.py` — Create ToolRegistry with all lookup methods + `build_registry()` factory.
...
- `src/agentpass/engine.py` — Add optional `registry` param to `validate_args()` and `build_signature()`. Add `PermissionEngine(permissions, registry=...)`.
...
- `src/agentpass/services/http.py` — Create GenericHTTPService implementing ServiceHandler.
- `src/agentpass/services/homeassistant.py` — Convert to deprecated wrapper.
...
- `src/agentpass/config.py` — Modify `load_config()` to parse generic ServiceConfig, call `load_tools_file()`.
- `src/agentpass/executor.py` — Remove TOOL_SERVICE_MAP, use ToolRegistry.
- `src/agentpass/server.py` — Add `list_tools` method handler.
- `src/agentpass/__main__.py` — Generic service init loop, registry construction, pass registry to engine/executor.
...
- `src/agentpass/cli.py` — Create: `run_request()`, `run_tools()`, `run_pending()`, `parse_key_value_args()`, exit code constants.
- `src/agentpass/__main__.py` — Restructure `parse_args()` with subparsers, update `main()` dispatch.
...
- [ ] `agentpass request ha_get_state entity_id=sensor.temp --url ... --token ...` prints JSON to stdout, exits 0
...
- [ ] `agentpass tools` prints JSON tool list
- [ ] `agentpass pending` prints JSON pending results
- [ ] `agentpass --insecure` still starts server (backward compat)
- [ ] `agentpass serve --insecure` also starts server
...
- `src/agentpass/__main__.py` — Add plugin loading logic in service init loop.
...
- `src/agentpass/engine.py` — Remove SIGNATURE_BUILDERS, HA_IDENTIFIER_RE, \_HA_IDENTIFIER_FIELDS constants.
- `src/agentpass/executor.py` — Remove TOOL_SERVICE_MAP constant.
...
- [ ] Use `agentpass tools` against running gateway
- [ ] Use `agentpass request ha_call_service domain=light service=turn_on entity_id=light.bedroom` against real HA
...
| CLI replaces mcporter for OpenClaw                      | agentpass's CLI is the tool interface. No need for mcporter as intermediary. Cleaner architecture.                                                      | 2026-02-08 |
